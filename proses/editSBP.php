...
$sql = "UPDATE buktipengeluaran SET noSPT=?, tahunAnggaran=?, tanggalInventaris=?, jumlahUang=?, untukPembayaran=?, rekening=?, tipePajak=?, pegawaiPajak=?, tipeBelanja=?, jumlahBelanja=?, penerimaPembayaran=?, penerimaBarang=?, bendaharaPengeluaran=?, kuasaAnggaran=?, nipPA=? WHERE noSBP=?";
$stmt = $conn->prepare($sql);
$stmt->bind_param('ssssssssssssssss', $noSPT, $tahunAnggaran, $tanggalInventaris, $jumlahUang, $untukPembayaran, $rekening, $tipePajak, $pegawaiPajak, $tipeBelanja, $jumlahBelanja, $penerimaPembayaran, $penerimaBarang, $bendaharaPengeluaran, $kuasaAnggaran, $nipPA, $noSBP);
