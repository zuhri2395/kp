...
$noSPT = (isset($_GET['noSPT']) ? $_GET['noSPT'] : null);
$sql = "SELECT * FROM spt WHERE noSPT='$noSPT'";
$query = $conn->query($sql);
$spt = $query->fetch_object();
$jmlhPeg = count(json_decode($spt->nip));
				...
				<form role="form" id="templatemo-preferences-form" method="POST" action="proses/editSPT.php">
					...
					<div class="row">
						<div class="col-md-12 margin-bottom-15">
							...
							<input type="text" id="noSPT" class="form-control" name="noSPT" placeholder="No Surat Perintah Tugas" value="<?= $spt->noSPT; ?>" readonly>
						</div>
					</div>
					...
					<div class="row">
						...
						<div class="col-md-6 margin-bottom-15">
							...
							<input type="text" class="form-control" id="noSPD" name="noSPD" placeholder="Nomor SPD" value="<?= $spt->noSPD; ?>" required>
						</div>

						<div class="col-md-6 margin-bottom-15">
							<label for="tanggalSPD" class="control-label">Tanggal SPD</label>
							<input type="text" class="form-control tanggal readonly>" id="tanggalSPD" name="tanggalSPD" placeholder="Tanggal Nomor SPD" value="<?= $spt->tanggalSPD; ?>" required>
						</div>
					</div>
					...
					<div class="row">
						<div class="col-md-12 margin-bottom-15">
							...
							<textarea class="form-control margin-bottom-15" rows="3" placeholder="Keterangan SPT" name="keterangan" id="keterangan" required><?= $spt->keterangan; ?></textarea>
						</div>
					</div>
					...
					<div class="row">
						<div class="col-md-6 margin-bottom-15">
							...
							<input class="form-control readonly tanggal" type="text" name="tanggalSPT" id="tanggalSPT" placeholder="Tanggal Keluar SPT" value="<?= $spt->tanggalSPT; ?>" required>
						</div>
						
						<div class="col-md-6 margin-bottom-15">
							<label for="kotaSPT" class="control-label">Dikeluarkan di Kota</label>
							<input class="form-control" type="text" name="kotaSPT" id="kotaSPT" value="<?= $spt->kotaSPT; ?>" placeholder="Dikeluarkan di Kota" required>
						</div>
					</div>
...
$(document).ready(function() {
	...
	var jmlhPeg = '<?= $jmlhPeg; ?>';
	$('#noPergub').val('<?= $spt->noPergub; ?>');
	$('#noDPA').val('<?= $spt->noDPA; ?>');
	$('#tanggalDinas').val('<?= $spt->tanggalDinas; ?>');
	$('#tanggalDinas').trigger("change");
	$('#jumlahPegawai').val('<?= $jmlhPeg; ?>');
	$('#jumlahPegawai').trigger("change");

	for(var i = 0; i < jmlhPeg; i++) {
		var nip = <?= $spt->nip; ?>;
		$('#optPegawai' + (i+1)).val(nip[i]);
	}

	$('#statusKadin').val('<?= $spt->statusKadin; ?>');
	$('#penandatanganSPT').val('<?= $spt->penandatanganSPT; ?>');
